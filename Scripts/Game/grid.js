...
export const WorldToGridPosition = function (grid, worldX, worldY) {
  if (grid != undefined) {
    ...
    if (
      worldX > grid.position.x + grid.size.x * grid.cellSize + grid.cellSize ||
      worldX < grid.position.x ||
      worldY > grid.position.y + grid.size.y * grid.cellSize + grid.cellSize ||
      worldY < grid.position.y
    ) {
      return undefined;
    }
